...
router.post("/", validatePost, (req, res) => {
  Listings.addListing(req.body)
    ...
    .catch(err =>
      res.status(500).json({ errMsg: "error posting listing", err: err })
    );
});
...
router.put("/:id", (req, res) => {
  Listings.editListing(req.body, req.params.id)
    ...
    .catch(err =>
      res.status(500).json({ errMsg: "error editing listing", err: err })
    );
});
...
router.get("/:userId", (req, res) => {
  ...
  if (userId != req.params.userId) {
    res
      ...
      .json({
        msg:
          "user not authorized (probably the given userId does not match the userId stored in the session)"
      });
  }
